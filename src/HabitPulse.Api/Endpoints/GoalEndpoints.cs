...
public static class GoalEndpoints
{
    public static void MapGoalEndpoints(this WebApplication app)
    {
                ...
                async (GoalService goalService, HttpContext context, CreateGoalRequest request) =>
                {
                    ...
                    if (request.IsMeasurable && request.TargetValue <= 0)
                        return Results.BadRequest(
                            new
                            {
                                error = "Target value must be greater than 0 for measurable goals",
                            }
                        );
        ...
        group
            .MapPost(
                "/reorder",
                async (GoalService goalService, HttpContext context, ReorderGoalsRequest request) =>
                {
                    var userId = context.User.GetUserId();
                    if (userId == null)
                        return Results.Unauthorized();
            ...
            .WithDescription(
                "Updates the sort order of goals based on the provided array of goal IDs."
            );
    }
}
